...
Améliorations vs v1 :
  ...
  - Grad clipping (norme 1.0) — inchangé
  - AdamW + L2 (weight_decay=1e-4) — inchangé
"""
...
# Plages autorisées pour le dropout adaptatif
_DROPOUT_MIN = 0.05
_DROPOUT_MAX = 0.50
_DROPOUT_STEP = 0.05
...
class TFTTrainer(BaseModel):
    ...
    def __init__(
        ...
        self._dropout    = dropout   # suivi courant pour le dropout adaptatif
    ...
    def fit(
        ...
        for epoch in range(self.epochs):
            ...
            if not has_val:
                if (epoch + 1) % 10 == 0:
                    logger.info(f"TFT Epoch {epoch+1}/{self.epochs} — train_loss={train_loss:.4f}")
                continue
            ...
            # Dropout adaptatif : overfitting si val_loss > 2× train_loss
            if val_loss > 2.0 * train_loss and self._dropout < _DROPOUT_MAX:
                ...
                logger.debug(f"TFT: dropout augmenté → {self._dropout:.2f} (val/train={val_loss/train_loss:.1f}x)")
            elif val_loss < 1.1 * train_loss and self._dropout > _DROPOUT_MIN:
                self._dropout = max(self._dropout - _DROPOUT_STEP, _DROPOUT_MIN)
                self.model.set_dropout(self._dropout)
                logger.debug(f"TFT: dropout réduit → {self._dropout:.2f}")
            ...
            if val_loss < best_val_loss - 1e-4:
                ...
                if no_improve >= patience:
                    logger.info(
                        f"TFT early stopping: epoch {epoch+1}  "
        ...
        if best_state is not None:
            ...
            logger.info("TFT: meilleurs poids restaurés après early stopping")
